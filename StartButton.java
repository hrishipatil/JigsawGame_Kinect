...
public class StartButton extends CommandButton implements IScreen
{
    private int x, y;
    ...
    int i=0;
    StartScreen receiver = new StartScreen();
    ...
    public void act() 
    {
        ...
        for(UserData user : users)
        {
            ...
            && flag ) // flag to select only 1 image in hand at a time
            {
                ...
                startCommand = new StartCommand(pw);
                startCommand.setReceiverScreen(receiver);
                startCommand.executeCommand();
            } 
        ...
        Label instr = new Label("Raise both hand to select or Drag image.(Mouse click/drag = raise left hand)", 20);
        //pw.addObject(instr, 50, 200);
        pw.addObject(instr, instr.getImage().getWidth() / 2, pw.getBackground().getHeight() - (instr.getImage().getHeight() / 2));
    }
